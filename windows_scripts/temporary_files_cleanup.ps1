...
# Self-elevating script with execution policy handling
if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    # Start a new PowerShell process with administrator privileges and bypass execution policy
    Start-Process PowerShell.exe -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$scriptPath`" $argString" -Verb RunAs
    exit
}

# Save current execution policy to restore it later
$originalExecutionPolicy = Get-ExecutionPolicy -Scope Process
try {
    # Temporarily set execution policy to bypass for this process only
    ...
    Write-Host "Execution policy restored to original state: $originalExecutionPolicy" -ForegroundColor Green
}
