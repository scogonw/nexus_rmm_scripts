...
# Script version - increment this when pushing new configs
$ScriptVersion = "1.0.0"
$ConfigTimestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
...
# Flag to track if configuration has been updated
$configUpdated = $false
...
try {
    ...
    # Check if existing config file has our version tracking
    $forceConfigUpdate = $true
    if (Test-Path $ServiceConfigPath) {
        Write-Status "Existing config file found, checking version..."
        $existingConfig = Get-Content $ServiceConfigPath -Raw -ErrorAction SilentlyContinue
        if ($existingConfig -match '# Version: (.+)') {
            $existingVersion = $matches[1]
            Write-VerboseLog "Existing config version: $existingVersion"
            
            if ($existingVersion -eq $ScriptVersion) {
                Write-Status "Config file is already at the latest version ($ScriptVersion)"
                # Still force update to ensure consistency
            }
        } else {
            Write-Status "Existing config has no version information, will be updated"
        }
        
        # Backup the old config
        $backupPath = "$ServiceConfigDir\config.yml.backup"
        Copy-Item -Path $ServiceConfigPath -Destination $backupPath -Force -ErrorAction SilentlyContinue
        Write-VerboseLog "Backed up existing config to $backupPath"
    }
    
    # Create new config with version tracking
    $configContent = @"
# NextDNS configuration file
# Generated by setup_nextdns.ps1
...
"@
    ...
    try {
        ...
        Write-Success "Configuration file created/updated in systemprofile location"
        Write-VerboseLog "Config file created at: $ServiceConfigPath"
        Write-VerboseLog "Config file content:`n$configContent"
        
        # Set a flag to indicate we need to restart the service due to config changes
        $configUpdated = $true
    ...
    # Create local config file in cloudflared directory
    $localConfigPath = "$InstallDir\config.yml"
    $configContent | Set-Content -Path $localConfigPath -Encoding ASCII -Force
    Write-VerboseLog "Created/updated local config file at: $localConfigPath"
    
    # Check again if service exists after possible deletion
    $serviceExists = Get-Service $ServiceName -ErrorAction SilentlyContinue
    
    if (-not $serviceExists) {
        # Install the service directly with sc.exe
        Write-Status "Creating cloudflared service with sc.exe..."
        $serviceExePath = "`"$ExePath`" --config `"$ServiceConfigPath`" proxy-dns"
        $scInstallOutput = sc.exe create $ServiceName binPath= $serviceExePath start= auto DisplayName= "$ServiceDisplayName"
        Write-CommandOutput "sc.exe create $ServiceName" $scInstallOutput
        
        if ($scInstallOutput -match "SUCCESS") {
            ...
            Write-Error "Failed to create service using sc.exe: $scInstallOutput"
            exit 1
        }
    }
    
    # If config was updated, restart the service to apply new configuration 
    if (($configUpdated -eq $true) -and ($serviceExists)) {
        Write-Status "Configuration was updated, restarting service to apply changes..."
        try {
            Restart-Service -Name $ServiceName -Force -ErrorAction Stop
            Start-Sleep -Seconds 5
            Write-Success "Service restarted successfully to apply new configuration"
        }
        catch {
            Write-Error "Failed to restart service using PowerShell: $_"
            Write-VerboseLog "Restart service error: $($_ | Format-List -Force | Out-String)"
            
            # Try using sc.exe to restart
            Write-Status "Attempting to restart service using sc.exe..."
            $scStopOutput = sc.exe stop $ServiceName
            Start-Sleep -Seconds 5
            $scStartOutput = sc.exe start $ServiceName
            Write-CommandOutput "sc.exe restart $ServiceName" "$scStopOutput`n$scStartOutput"
            Start-Sleep -Seconds 5
        }
    }
    ...
    if (-not $serviceCheck) {
        Write-Error "Service does not exist after creation/update steps"
        exit 1
    }
    ...
    if ($runningService -and $runningService.Status -eq 'Running') {
        ...
        # Check for existing service using different methods to ensure we catch it
        $existingService = Get-Service $ServiceName -ErrorAction SilentlyContinue
        $existingServiceByDisplayName = Get-Service -DisplayName "*$ServiceDisplayName*" -ErrorAction SilentlyContinue
        
        if ($existingService -or $existingServiceByDisplayName) {
            $serviceToUse = if ($existingService) { $existingService } else { $existingServiceByDisplayName }
            $ServiceName = $serviceToUse.Name
            
            Write-Status "Service '$ServiceName' already exists. Checking status..."
            Write-VerboseLog "Existing service details: $($serviceToUse | Format-List -Force | Out-String)"
            
            if ($serviceToUse.Status -eq 'Running') {
                Write-Status "Service is already running. Stopping it before reconfiguration..."
                try {
                    Stop-Service -Name $ServiceName -Force -ErrorAction Stop
                    Write-Success "Service stopped successfully"
                } catch {
                    Write-Error "Failed to stop service using PowerShell: $_"
                    # Try with sc.exe
                    $scStopOutput = sc.exe stop $ServiceName
            ...
            # Check if the service binPath is correct
            $scQueryConfig = sc.exe qc $ServiceName
            Write-VerboseLog "Service configuration: $scQueryConfig"
            
            # Update the service configuration to ensure it uses our config
            Write-Status "Updating service configuration..."
            $serviceExePath = "`"$ExePath`" --config `"$ServiceConfigPath`" proxy-dns"
            $scConfigResult = sc.exe config $ServiceName binPath= $serviceExePath start= auto DisplayName= "$ServiceDisplayName"
            Write-CommandOutput "sc.exe config $ServiceName" $scConfigResult
            
            if ($scConfigResult -match "SUCCESS") {
                Write-Success "Service configuration updated successfully"
            } else {
                Write-Error "Failed to update service configuration. Will attempt to recreate service..."
                
                # Try to delete the service and recreate it
                $scDeleteOutput = sc.exe delete $ServiceName
                Write-CommandOutput "sc.exe delete $ServiceName" $scDeleteOutput
                Start-Sleep -Seconds 5
                
                # Service will be recreated in the next step
            }
        }
        ...
        if ($runningService -and $runningService.Status -eq 'Running') {
            ...
            # Try to run in console mode to see any errors
            Write-Status "Testing cloudflared in console mode..."
            $testConsoleOutput = & $ExePath service run 2>&1
            Write-CommandOutput "cloudflared service run" $testConsoleOutput
            
            # Warning about service not running
    ...
    if (-not $portCheck) {
        ...
        if (-not $portCheck) {
            ...
            if (Test-Path $ServiceConfigPath) {
                ...
                try {
                    $configAcl = Get-Acl -Path $ServiceConfigPath -ErrorAction SilentlyContinue
                    Write-VerboseLog "Config file ACL: $($configAcl | Format-List -Force | Out-String)"
                    ...
                    Set-Acl -Path $ServiceConfigPath -AclObject $configAcl -ErrorAction SilentlyContinue
                    Write-Status "Ensured proper permissions on config file"
                } catch {
                    Write-VerboseLog "Error updating config file permissions: $_"
                }
            } else {
            ...
            # Check port again after all the restart attempts
            $finalPortCheck = netstat -ano | Select-String ":53 "
            if ($finalPortCheck) {
                Write-Success "Port 53 is now in use after service restart: $finalPortCheck"
            } else {
                Write-Error "Port 53 is still not in use after all attempts"
                
                # Try direct console execution as a last resort diagnostic
                Write-Status "Attempting to run cloudflared directly in console mode as diagnostic..."
                $diagnosticOutput = Start-Process -FilePath $ExePath -ArgumentList "proxy-dns", "--config", "$ServiceConfigPath" -NoNewWindow -PassThru
                Write-Status "Started diagnostic process with PID: $($diagnosticOutput.Id)"
                # Wait briefly
                Start-Sleep -Seconds 5
                # Check if the process is still running
                if (-not $diagnosticOutput.HasExited) {
                    Write-Success "Diagnostic process is still running, which suggests cloudflared can run properly"
                    Write-Status "You may need to try restarting the computer to get the service working properly"
                    # Kill the diagnostic process
                    try {
                        $diagnosticOutput.Kill()
                    } catch {
                        Write-VerboseLog "Error killing diagnostic process: $_"
                    }
                } else {
                    Write-Error "Diagnostic process exited immediately with code: $($diagnosticOutput.ExitCode)"
                }
            }
        }
    } else {
        ...
        if ($adapters.Count -eq 0) {
            ...
            foreach ($adapter in $adapters) {
                ...
                if (-not $ipv6Components.Enabled) {
                    ...
                    Write-Success "  IPv6 is already enabled on adapter: $($adapter.Name)"
                }
            }
        ...
        if ($adapters.Count -eq 0) {
            ...
            foreach ($adapter in $adapters) {
                ...
                try {
                    ...
                    Write-Success "  IPv4 DNS configured for adapter: $($adapter.Name)"
                } catch {
                    Write-Error "  Failed to set IPv4 DNS for adapter $($adapter.Name): $_"
                    Write-VerboseLog "IPv4 DNS setting error: $($_ | Format-List -Force | Out-String)"
                    
                    # Try alternative method using netsh
                    try {
                        $interfaceName = $adapter.Name
                        $quotedInterfaceName = "`"$interfaceName`""
                        $netshResult = netsh interface ipv4 set dns $quotedInterfaceName static 127.0.0.1 primary
                        Write-CommandOutput "netsh interface ipv4 set dns" $netshResult
                        Write-Success "  IPv4 DNS configured for adapter: $($adapter.Name) using netsh"
                    } catch {
                        Write-Error "  Failed to set IPv4 DNS using netsh for adapter $($adapter.Name): $_"
                        Write-VerboseLog "netsh IPv4 error: $($_ | Format-List -Force | Out-String)"
                    }
                }
    ...
    # 6.5 Configure firewall rules for DNS traffic
    Write-Status "Configuring Windows Firewall rules for DNS traffic..."
    try {
        # Check if rules already exist (for idempotency)
        $existingRuleIPv4 = Get-NetFirewallRule -DisplayName "Allow Cloudflared DNS (IPv4)" -ErrorAction SilentlyContinue
        $existingRuleIPv6 = Get-NetFirewallRule -DisplayName "Allow Cloudflared DNS (IPv6)" -ErrorAction SilentlyContinue
        
        # IPv4 rule
        if (-not $existingRuleIPv4) {
            Write-Status "Creating IPv4 firewall rule..."
            New-NetFirewallRule -DisplayName "Allow Cloudflared DNS (IPv4)" -Direction Inbound -Protocol UDP `
                -LocalPort 53 -Action Allow -Profile Any -Program $ExePath -Description "Allow DNS queries on port 53" -ErrorAction Stop | Out-Null
            Write-Success "Created IPv4 firewall rule"
        } else {
            Write-Status "IPv4 firewall rule already exists"
        }
        
        # IPv6 rule
        if (-not $existingRuleIPv6) {
            Write-Status "Creating IPv6 firewall rule..."
            New-NetFirewallRule -DisplayName "Allow Cloudflared DNS (IPv6)" -Direction Inbound -Protocol UDP `
                -LocalPort 53 -Action Allow -Profile Any -Program $ExePath -Description "Allow DNS queries on port 53 (IPv6)" -ErrorAction Stop | Out-Null
            Write-Success "Created IPv6 firewall rule"
        } else {
            Write-Status "IPv6 firewall rule already exists"
        }
        
        # TCP rules for potential DNS over TCP
        $existingRuleTCPv4 = Get-NetFirewallRule -DisplayName "Allow Cloudflared DNS TCP (IPv4)" -ErrorAction SilentlyContinue
        $existingRuleTCPv6 = Get-NetFirewallRule -DisplayName "Allow Cloudflared DNS TCP (IPv6)" -ErrorAction SilentlyContinue
        
        # TCP IPv4 rule
        if (-not $existingRuleTCPv4) {
            Write-Status "Creating TCP IPv4 firewall rule..."
            New-NetFirewallRule -DisplayName "Allow Cloudflared DNS TCP (IPv4)" -Direction Inbound -Protocol TCP `
                -LocalPort 53 -Action Allow -Profile Any -Program $ExePath -Description "Allow DNS queries over TCP on port 53" -ErrorAction Stop | Out-Null
            Write-Success "Created TCP IPv4 firewall rule"
        }
        
        # TCP IPv6 rule
        if (-not $existingRuleTCPv6) {
            Write-Status "Creating TCP IPv6 firewall rule..."
            New-NetFirewallRule -DisplayName "Allow Cloudflared DNS TCP (IPv6)" -Direction Inbound -Protocol TCP `
                -LocalPort 53 -Action Allow -Profile Any -Program $ExePath -Description "Allow DNS queries over TCP on port 53 (IPv6)" -ErrorAction Stop | Out-Null
            Write-Success "Created TCP IPv6 firewall rule"
        }
        
        # Outbound rule for DoH traffic (HTTPS)
        $existingOutboundRule = Get-NetFirewallRule -DisplayName "Allow Cloudflared Outbound HTTPS" -ErrorAction SilentlyContinue
        if (-not $existingOutboundRule) {
            Write-Status "Creating outbound HTTPS firewall rule..."
            New-NetFirewallRule -DisplayName "Allow Cloudflared Outbound HTTPS" -Direction Outbound -Protocol TCP `
                -RemotePort 443 -Action Allow -Profile Any -Program $ExePath -Description "Allow outbound HTTPS for DoH" -ErrorAction Stop | Out-Null
            Write-Success "Created outbound HTTPS firewall rule"
        }
        
    } catch {
        Write-Error "Failed to configure firewall rules: $_"
        Write-VerboseLog "Firewall configuration error: $($_ | Format-List -Force | Out-String)"
        # Continue script execution, as firewall rules are helpful but not critical
    }
    ...
    Write-Host "Configuration Timestamp: $ConfigTimestamp" -ForegroundColor Yellow
